...
Plex's /config lives in a Docker named volume (not the host filesystem),
so Preferences.xml is pulled out via `docker cp`, rewritten locally, and
copied back in when a change is needed.
"""
...
PLEX_PREFS_CONTAINER_PATH = "/config/Library/Application Support/Plex Media Server/Preferences.xml"
...
def _cp_out_prefs(dst):
    """docker cp plex:Preferences.xml -> dst (local path). Returns True on success."""
    result = subprocess.run(
        ["docker", "cp", f"plex:{PLEX_PREFS_CONTAINER_PATH}", dst],
        capture_output=True, timeout=30, env=config.BREW_ENV,
    )
    return result.returncode == 0


def _cp_in_prefs(src):
    """docker cp src (local path) -> plex:Preferences.xml."""
    result = subprocess.run(
        ["docker", "cp", src, f"plex:{PLEX_PREFS_CONTAINER_PATH}"],
        capture_output=True, timeout=30, env=config.BREW_ENV,
    )
    return result.returncode == 0
...
def sync_custom_connections():
    ...
    try:
        if not _cp_out_prefs(str(tmp_path)):
            log.debug("Plex network: could not read Preferences.xml from container")
            return
        ...
        if not _cp_in_prefs(str(tmp_path)):
            log.error("Plex network: failed to copy Preferences.xml back into plex container")
            subprocess.run(
